...
from vehicle.builders import VehicleBuilder
import logging
import json
...
class VehicleListByCarView(generics.ListAPIView):
    ...
    queryset = (
        Vehicle.objects.all().order_by("brand").filter(type_vehicle=TypeVehicle.CAR)
    )
    serializer_class = VehicleSerializer
...
class VehicleListByMotoView(generics.ListAPIView):
    ...
    queryset = (
        Vehicle.objects.all()
        .order_by("brand")
        .filter(type_vehicle=TypeVehicle.MOTORCYCLE)
    )
    serializer_class = VehicleSerializer
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            if Vehicle.objects.filter(
                ...
                return Response(
                    {"message": "Modelo já registrado.", "result": serializer.data},
            ...
            return Response(
                {"message": "Veículo criado com sucesso!", "result": serializer.data},
                status=status.HTTP_201_CREATED,
            )
        except Exception as e:
            return Response(
                {"message": "Erro ao criar veículo!", "error": str(e)},
...
class VehicleDeleteView(generics.DestroyAPIView):
    ...
    def delete(self, request, *args, **kwargs):
        try:
            ...
            return Response(
                {"error": "Veiculo não encontrado."}, status=status.HTTP_404_NOT_FOUND
            )
        except Exception as e:
            return Response(
                {"error": f"Erro ao excluir Veiculo: {str(e)}"},
                status=status.HTTP_400_BAD_REQUEST,
            )
